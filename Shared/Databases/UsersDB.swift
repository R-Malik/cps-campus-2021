...
class UserFetcher: ObservableObject, Equatable {
    ...
    func observeData(emailID: String) {
        ...
        reference = Database.database(url: "").reference().child("users").child(cleanFirebaseKey(input: emailID))
    ...
    func observeCourses(emailID: String) {
        var reference: DatabaseReference!
        reference = Database.database(url: "").reference().child("courses").child(cleanFirebaseKey(input: emailID))
...
func fetchAllUsers(completion: @escaping ([User])->()) {
    var reference: DatabaseReference!
    reference = Database.database(url: "").reference().child("users")
