...
class ClubsFetcher: ObservableObject, Equatable {
    ...
    init() {
        ...
        reference = Database.database(url: "").reference()
...
func publishClubLink(link: QuickLink, clubName: String) {
    var reference: DatabaseReference!
    reference = Database.database(url: "").reference().child(cleanFirebaseKey(input: clubName)).child("links")
